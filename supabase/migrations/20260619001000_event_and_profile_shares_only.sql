...
begin
  ...
  if p_scope = 'album' then
    raise exception 'Albums are shared through their parent event or photographer page';
  end if;
...
begin
  ...
  if not exists (
    ...
    where is_active = true
      ...
      and (
        (target_event.id is not null and scope = 'event' and event_id = target_event.id)
  ...
  using (
    ...
    and exists (
      ...
      where photos.storage_path = storage.objects.name
        ...
        and (
          (share_links.scope = 'event' and share_links.event_id = events.id)
